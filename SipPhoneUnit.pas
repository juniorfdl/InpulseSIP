...
type
  ...
  public
    ...
    class function Ligar(pFone:WideString):WideString;
...
begin
  result := '0';
  SipPhoneForm.StartCallBtn.Click;
  result := '1';
end;

class function TSipPhoneForm.Ligar(pFone: WideString): WideString;
begin
  result := '0';
  SipPhoneForm.AddressComboBox.Text := pFone;
  SipPhoneForm.StartCallBtn.Click;
  result := '1';
end;
...
begin
    ...
    if (isCalledByPlayFinishedEvent) and (not lnInfo.IsCallPlayStarted )
        then Exit;
    if lnInfo.IsCallPlayStarted
        then begin
            AbtoPhone.StopPlayback;
            PlayButton.Caption := 'Play';
        end else begin
            OpenDialog.Options := OpenDialog.Options - [ofAllowMultiSelect];
            OpenDialog.Filter := 'Sound Files (*.wav)|*.wav';
            if not OpenDialog.Execute
                then Exit;
            succeded := AbtoPhone.PlayFile(OpenDialog.FileName);
            if succeded = 0
                then Exit;
            Self.DisplayNotifyMessage('Now playing: ' + OpenDialog.FileName);
...
var
    ...
    str: WideString;
begin
    fStatus := '{"Evento": "OnClearedCall", "Msg":"'+Msg+'", "LineId": "'+IntToStr(LineId)+'", "Time":"'+TimeToStr(Now)+'" }';
...
begin
  ...
  if Self.Width > CallPanel.Width + 2*deltaWidth Then  begin
      //Straiten
